#!/bin/bash
# Test: Basic Minimal image creation
# Download latest Minimal image and create a custom disk image with minimal options
...
echo "======================================================"
echo "= Test: Minimal Image - Basic Creation"
echo "= Started: $(date)"
echo "======================================================"
...
echo "= Step 1: Downloading Minimal image..."
MINIMAL_IMAGE=$("${HELPERS_DIR}/download-images.sh" minimal)

if [ -z "$MINIMAL_IMAGE" ] || [ ! -f "$MINIMAL_IMAGE" ]; then
    echo "Error: Failed to download Minimal image"
    exit 1
fi

if [ ! -r "$MINIMAL_IMAGE" ]; then
    echo "Error: Minimal image is not readable: $MINIMAL_IMAGE"
    exit 1
fi

echo "= Using image: $MINIMAL_IMAGE"

# Create output file (empty file for arm-image-installer to write to)
OUTPUT_IMAGE="${TEST_OUTPUT_DIR}/test-minimal.img"
rm -f "$OUTPUT_IMAGE"
...
echo "= Step 3: Verifying image structure..."

if ! "${HELPERS_DIR}/verify-image.sh" "$OUTPUT_IMAGE"; then
...
echo "======================================================"
echo "= ✓ Test PASSED: Minimal Basic"
