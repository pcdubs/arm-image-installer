...
# Note: For IoT images with ignition, we verify the image was created
# Ignition files are processed at boot time, so we just verify basic structure
if ! "${HELPERS_DIR}/verify-image.sh" "$OUTPUT_IMAGE"; then
    echo "Error: Image verification failed"
    exit 1
fi

# Additional check: verify ignition file was embedded (if applicable)
echo "= Note: Ignition configuration is processed at boot time"
echo "= Basic image structure verification passed"
