...
echo "= DEBUG: SERVER_IMAGE='$SERVER_IMAGE'" >&2
echo "= DEBUG: SERVER_IMAGE length=${#SERVER_IMAGE}" >&2
if [ -n "$SERVER_IMAGE" ]; then
    echo "= DEBUG: File exists check: $([ -f "$SERVER_IMAGE" ] && echo YES || echo NO)" >&2
    echo "= DEBUG: ls output:" >&2
    ls -lh "$SERVER_IMAGE" 2>&1 >&2 || echo "  (file not found)" >&2
fi
