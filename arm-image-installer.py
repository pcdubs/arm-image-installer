...
from modules import partition, resize, config, uboot
...
def parse_args():
    ...
    required.add_argument('--image', required=True, help="Path to compressed disk image (.xz or raw only)")
    required.add_argument('--media', required=True, help="Target block device (e.g., /dev/sdX, /dev/mmcblkX, /dev/nvmeXnY)")
    required.add_argument('--target', required=True, help="Target board name or alias")
...
def main():
    ...
    print_summary_and_confirm(args, iot_image)
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        if args.dry_run:
            ...
            with partition.mounted_partition(boot_part, boot_mount, args.dry_run), \
                ...
                uboot.install_uboot(args.target, root_mount, dry_run=args.dry_run)
